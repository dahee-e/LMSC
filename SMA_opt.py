...
def getChains(G, Core, h1):
    ...
    for u in P:
        chain = [u]
        clsm_calc = cu.CLSMCalculator(G, C_nodes, chain)
        clsm = clsm_calc.get_clsm()
        while len(chain) < h1:
            neighbors = set(cu.get_neighbour(G, set(chain))) - C_nodes
            if len(neighbors) == 0:
                break
            v = max(neighbors, key=lambda w: (
                clsm_calc.get_clsm_append_node(w),
                -nx.shortest_path_length(G, chain[0], w),
                -int(w)
            ))

            clsm_append = clsm_calc.get_clsm_append_node(v)
            if clsm_append < clsm:
                break
            chain.append(v)
            clsm_calc.add_node(v)
            clsm = clsm_append
        Z.append(chain)
    return Z, P
...
def updateChains(G, C, S, Z, P_prime, h1): # G: 전체 그래프, C: 현재 그래프, SS: subchain, h1: 최대 길이, Z: chain set
    ...
    for chain in Z:
        ...
        N_S = set(cu.get_neighbour_include_set(G, S))  # N(S,G)
        ...
        clsm_calc = cu.CLSMCalculator(G, C_nodes, new_chain)
        clsm = clsm_calc.get_clsm()
        ...
        while len(new_chain) < h1:  # Extend chain greedily
            ...
            if not candidates:
                break
            best_v = max(candidates, key=lambda w: (clsm_calc.get_clsm_append_node(w), -nx.shortest_path_length(G, pivot, w), -int(w)))
            clsm_append = clsm_calc.get_clsm_append_node(best_v)
            if clsm_append < clsm:
                break
            clsm = clsm_append
            new_chain.append(best_v)
            clsm_calc.add_node(best_v)
    ...
    for pivot in new_pivots:
        ...
        while len(new_chain) < h1:
            ...
            if not candidates:
                break
            best_v = max(candidates, key=lambda w: (
                clsm_calc.get_clsm_append_node(w),
                -nx.shortest_path_length(G, pivot, w),
                -int(w)
            ))
...
def findBestSubchain(G, C, Z, t, h_prime, q_nodes):
    """최적화된 Subchain 탐색"""
    C_nodes = set(C.graph.nodes())
    S_max = []
    lsm_max = -1
    ...
    for index, T in enumerate(Z):
        Subchain = []
        current_nodes = C_nodes.copy()
        temp_calc = cu.LSMCalculator(G, C_nodes, t)

        for v in T:
            current_nodes.add(v)
            Subchain.append(v)

            if v not in temp_calc.node_stats:
                temp_calc.append_node_stats(v)

            lsm_current = temp_calc.get_lsm_append_node(v)


            if lsm_max == lsm_current and S_max != []:
                cand = [S_max, Subchain]
                S_max = min(cand, key=lambda x: (
                    min(nx.shortest_path_length(G, q_node, x[0]) for q_node in q_nodes),
                    len(x),
                    int(x[0])
                )).copy()
...
def run(G, q, l, h, t,steiner_type):
    ...
    while C.size < h:
        ...
        v = max(neighbours, key=lambda x: (
            lsm_calc.get_lsm_append_node(x),
            -min(nx.shortest_path_length(G, q_node, x) for q_node in q),
            -int(x)
        ))

        lsm_append = lsm_calc.get_lsm_append_node(v)

        if lsm_append > C.lsm_value:
            ...
            C = SubgraphData(community, lsm_calc.get_lsm(), len(community), C.sequence)
